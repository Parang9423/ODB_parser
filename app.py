...
class ODBCamApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(preview_frame, text="CAM 미리보기", font=("TkDefaultFont", 12, "bold")).pack(anchor="w", pady=(0, 8))
        self.preview_canvas = tk.Canvas(preview_frame, background="#202020", highlightthickness=0)
    ...
    def _draw_preview(self) -> None:
        ...
        if self.preview_image is None:
            ...
            return
        cw = max(1, self.preview_canvas.winfo_width() - 24)
        ch = max(1, self.preview_canvas.winfo_height() - 24)
        image = self.preview_image.copy()
        image.thumbnail((min(cw, PREVIEW_MAX_SIZE[0]), min(ch, PREVIEW_MAX_SIZE[1])), Image.Resampling.LANCZOS)
    ...
    def _update_info(self) -> None:
        ...
        if step:
            try:
                ...
                pass
        if layer:
            lines.extend(["", f"Layer: {layer.name}", f"Type: {layer.layer_type}", f"Context: {layer.context}", f"Side: {layer.side}", f"Polarity: {layer.polarity}", "", f"Preview DPI: {PREVIEW_DPI}", f"Output DPI: {self.dpi_var.get()}"])
