...
class LayerDialog(tk.Toplevel):
    def __init__(self, parent: "App", layers: List[LayerInfo], selected: set[str]):
        ...
        # X11/xrdp: map the window before taking the grab, otherwise it can exist invisibly.
...
class App(_CoreApp):
    def _ui(self):
        ...
        top=self.file_label.master;self.layer_pick_btn=ttk.Button(top,text="레이어 선택/변경",command=self.choose_layers,state="disabled");self.layer_pick_btn.pack(side="left",padx=(0,8),before=self.file_label);ttk.Label(top,textvariable=self.pnl_coord_text).pack(side="right");self.canvas.bind("<Motion>",self.on_motion,add="+");self.canvas.bind("<Leave>",lambda _e:self.pnl_coord_text.set("PNL X: - mm   Y: - mm"),add="+");self.render_btn.configure(text="Composite 결과 저장")
    def apply_loaded(self,path,job,tmp,info):
        if self.tmp:self.tmp.cleanup()
        self.job,self.tmp,self.info,self.source=job,tmp,info,path;self.selected_specs.clear();self.preview=None;self._hier_profiles=[];self.file_label.configure(text=path.name);self.layer_pick_btn.configure(state="normal");available={x.lower() for x in info.steps}
    ...
    def render(self):
        ...
        def work():
            try:
                ...
                # CAM is sparse and compresses well already. Low zlib level is much faster than optimize=True and remains lossless.
